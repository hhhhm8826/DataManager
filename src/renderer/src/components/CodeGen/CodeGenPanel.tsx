...
export function CodeGenPanel(): React.JSX.Element {
  ...
  const handleGenerate = async (lang: string): Promise<void> => {
    ...
    if (lang === 'unreal') {
      ...
      setGenerating(null)
      if (result.success && result.data)
        toast.success(`Unreal C++ 헤더 ${result.data.length}개 생성 완료`)
  ...
  const handleGenerateAll = async (): Promise<void> => {
    ...
    if (protocConfiguredCount > 0) {
      ...
      if (protocResult.success && protocResult.data)
        results.push(...protocResult.data.map(getLabel))
  ...
  const protocConfiguredCount = outputDirs.filter(
    (o) => o.language !== 'unreal' && o.dir.trim()
  ).length
      ...
      <div className="page-body">
          ...
          <div style={{ marginBottom: 16 }}>
            <button
              ...
              disabled={
                ...
                (protocConfiguredCount > 0 && !protocPath)
              }
              style={{ fontSize: 14, padding: '8px 20px' }}
            >
              {generating === 'all'
                ? '생성 중...'
                : `⚡ 전체 생성 (${totalConfiguredCount}개 언어)`}
            </button>
          </div>
        ...
        <div className="card">
          ...
          {availableLanguages.map((lang) => (
            <div key={lang} className="form-group">
              ...
              <div className="path-row">
                ...
                />
                <button className="btn btn-ghost" onClick={() => pickDir(lang)} title="폴더 선택">
                  📂
                </button>
                <button
                  className="btn btn-ghost"
                  title="탐색기에서 열기"
                  onClick={() => openDir(getDirForLang(lang))}
                >
                  ↗
                </button>
              </div>
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <div className="path-row">
              ...
              />
              <button className="btn btn-ghost" onClick={() => pickDir('unreal')} title="폴더 선택">
                📂
              </button>
              <button
                className="btn btn-ghost"
                title="탐색기에서 열기"
                onClick={() => openDir(unrealOutputDir)}
              >
                ↗
              </button>
            </div>
          ...
          {dirty && (
            <div className="toolbar" style={{ marginTop: 12 }}>
              <button className="btn btn-success" onClick={handleSavePaths}>
                💾 저장
              </button>
            </div>
          )}
        ...
        <div className="card">
          ...
          <p style={{ fontSize: 13, color: '#9ca3af', marginBottom: 12 }}>
            Protocol Buffers 컴파일러(<code style={{ color: '#a0c4ff' }}>protoc</code>) 실행 파일
            경로를 지정합니다.&nbsp;
            <a
          ...
          <p style={{ fontSize: 13, color: '#9ca3af', marginBottom: 12 }}>
            Go는 <code style={{ color: '#a0c4ff' }}>protoc-gen-go</code>, Rust는{' '}
            <code style={{ color: '#a0c4ff' }}>protoc-gen-rust</code> 플러그인이 PATH에 있어야
            합니다.
          </p>
          <div className="form-group" style={{ marginBottom: 0 }}>
