...
처음 실행하면 포함된 예제 프로젝트가 사용자 데이터 폴더에 복사되고 Proto,
Excel, JSON, 코드 출력과 protoc 경로가 자동으로 설정됩니다. 별도 설정 없이
화면과 생성 흐름을 먼저 확인할 수 있으며, 예제 원본은 앱 리소스와 분리되어
자유롭게 수정해도 됩니다.

1. **테이블**, **Enum**, **관계도**에서 준비된 예제 구조와 참조를 확인합니다.
2. **Excel**에서 필요한 파일을 선택해 입력 양식을 생성합니다.
3. Excel에 데이터를 입력한 뒤 **JSON 생성**으로 값을 검증하고 출력합니다.
4. 개발 환경에 필요한 언어를 선택해 **코드 생성**을 실행합니다.
5. 실제 프로젝트를 시작할 때 **설정**에서 작업 폴더를 변경합니다.
...
```text
target/release/bundle/nsis/DataManager_0.1.1_x64-setup.exe
```

GitHub Actions에서는 설치판과 함께 압축을 풀고 바로 실행할 수 있는 일반
Portable ZIP도 생성합니다.

```text
target/release/bundle/nsis/DataManager_0.1.1_x64-setup.exe
target/release/bundle/portable/DataManager_0.1.1_windows_x64_portable.zip
```

Portable판은 Windows 10/11의 Microsoft Edge WebView2를 사용하며 설치 과정은
필요하지 않습니다. 두 배포 형식 모두 격리된 사용자 프로필에서 첫 실행 예제
설정과 앱 실행을 검증합니다.
